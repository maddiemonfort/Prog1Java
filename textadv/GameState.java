...
public class GameState {
	...
	public String enterRoom() { //or exit room
		if(currentRoom.getName().equals("Kronos")) {
			if(talkedToAriadne && stringUsed) {
				...
				System.out.println("Both you and Ariadne escape the maze and you return her to her family. After the exhuberant welcome, you and Ariadne travel to your home of Athens to bring the good news to your father...and the rest is mythology.");
				return "end";
			}
			else {
		...
		else if(currentRoom.getName().equals("entrance")) {
			if(talkedToDaedalus && currentRoom.open()) {
				currentRoom = getNextRoom();
				return "entering the Temple of " + currentRoom.getName();
			}
			else {
				return "You may not leave yet. There is a man watching you.";
			}
		}
		else {
			if(currentRoom.open()) {
	...
	public String useNectar() {
		if(inventory.hasItem("nectar")) {
			player.setHealth(player.getHealth() + 3);
			inventory.removeItem("nectar");
			return "You have been healed.";
		}
		else {
			return "You cannot use something you don't have.";
		}
	}

	public String readBook() {
		if(inventory.hasItem("book")) {
			return "You flip to a chapter titled, 'Riddles in the Dark', which interests you. Skimming it, you find that Bilbo has a ring in his pocket.";
		}
		else {
			return "You do not have a book to read.";
		}
	}

	public String dropItem(String i) {
